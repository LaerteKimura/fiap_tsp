...
VRP_FIXED_COST_PER_ROUTE = 800  # Custo fixo em R$ adicionado ao custo de cada rota

# Critérios de parada VRP - limites configuráveis para controle da evolução
VRP_MAX_GENERATIONS = 1000  # Máximo de gerações para VRP (evita execução infinita)
VRP_CONVERGENCE_GENERATIONS = 50  # Gerações sem melhoria para considerar convergência no VRP
VRP_CONVERGENCE_THRESHOLD = 0.01  # Melhoria mínima para resetar contador de convergência no VRP
